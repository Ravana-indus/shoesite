...
export default function ProductForm({ product, brands, categories, onSave, onClose }: ProductFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      let saved: Product;
      if (product) {
        saved = await api.products.update(product.id, form);
      } else {
        saved = await api.products.create(form as Parameters<typeof api.products.create>[0]);
      }
      for (const img of images) {
        if (img.url && !product?.images?.some(p => p.url === img.url)) {
          await api.products.addImage(saved.id, img.url, img.alt_text || (img.url.split('/').pop() ?? ''));
        }
      }
      onSave();
    } catch (err: unknown) {
  ...
  async function handleDeleteImage(img: ProductImage) {
    if (!product) return;
    try {
      await api.products.removeImage(img.id);
      ...
      <form onSubmit={handleSubmit} className="overflow-y-auto max-h-[calc(90vh-120px)] p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            />
            {form.price_cents > 0 && (
              <p className="text-xs text-on-surface-variant mt-1">= {formatPriceCents(form.price_cents)}</p>
            )}
          </div>

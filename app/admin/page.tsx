...
export default async function AdminDashboard() {
  ...
  const [ordersData, lowStockData] = await Promise.all([
    ...
    supabase.from('products').select('*').lt('stock_qty', 5).limit(10),
  ]);
  ...
  const thisMonthRevenue = thisMonthOrders.reduce((sum, o) => sum + (o.payment_status === 'paid' ? (o.total_cents ?? 0) : 0), 0);
  const prevMonthRevenue = prevMonthOrders.reduce((sum, o) => sum + (o.payment_status === 'paid' ? (o.total_cents ?? 0) : 0), 0);
  ...
  return (
    <div className="min-h-screen bg-surface-container-low text-on-surface p-4 lg:p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/" className="text-sm text-secondary hover:text-primary flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-surface rounded-xl border border-outline-variant/10 overflow-hidden">
          <div className="p-6 border-b border-outline-variant/10 flex items-center justify-between">
            ...
            <Link href="/admin/orders" className="text-xs text-secondary uppercase tracking-widest hover:text-primary">View All</Link>
          </div>
        ...
        <div className="bg-surface rounded-xl border border-outline-variant/10 overflow-hidden">
          <div className="p-6 border-b border-outline-variant/10 flex items-center justify-between">
            ...
            <Link href="/admin/products" className="text-xs text-secondary uppercase tracking-widest hover:text-primary">Manage</Link>
          </div>
          <div className="divide-y divide-outline-variant/5">
